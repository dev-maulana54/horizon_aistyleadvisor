  ...
  <style>
    * {
    ...
    .feature-card:hover {
      ...
      box-shadow: 0 12px 24px rgba(123, 30, 43, 0.15);
    }
...
<body class="h-full light-mode">
  <div id="app" class="h-full flex flex-col lg:flex-row overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <div id="page-tryon" class="page-content hidden p-4 lg:p-8 fade-in">
          <div
            class="card-bg rounded-2xl p-4 lg:p-6 shadow-md mb-4 flex items-center justify-between sticky top-0 z-10 border">
            <div class="flex items-center gap-3">
              <button
            ...
            <button
              type="button"
              onclick="showPage('chat')"
              class="px-4 py-2 rounded-xl border text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              Ask in Chat
            </button>
          </div>
          ...
          <div class="grid lg:grid-cols-3 gap-4">
            <div class="lg:col-span-2 card-bg rounded-2xl p-5 shadow-md">
              ...
              <div class="grid sm:grid-cols-2 gap-4">
                <div class="border rounded-2xl p-4">
                  ...
                  <label class="block cursor-pointer">
                    <input
                      id="tryon-person-input"
                      type="file"
                      accept="image/*"
                      class="hidden"
                      onchange="handleTryOnUpload(event, 'person')" />
                    <div
                      class="border-2 border-dashed rounded-2xl p-4 text-center hover:border-current transition-colors">
                  ...
                  <div
                    class="mt-3 rounded-2xl overflow-hidden bg-black/5 border aspect-[3/4] flex items-center justify-center">
                    <img
                      id="tryon-person-preview"
                      src=""
                      alt="Person Preview"
                      class="w-full h-full object-contain hidden" />
                    <div
                      id="tryon-person-placeholder"
                      class="text-xs text-muted p-3 text-center">
                ...
                <div class="border rounded-2xl p-4">
                  ...
                  <label class="block cursor-pointer">
                    <input
                      id="tryon-outfit-input"
                      type="file"
                      accept="image/*"
                      class="hidden"
                      onchange="handleTryOnUpload(event, 'outfit')" />
                    <div
                      class="border-2 border-dashed rounded-2xl p-4 text-center hover:border-current transition-colors">
                  ...
                  <div class="mt-3">
                    ...
                    <select
                      id="tryon-wardrobe-select"
                      class="mt-2 w-full px-3 py-2 card-bg border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-opacity-50"
                      onchange="selectOutfitFromWardrobe()">
                  ...
                  <div
                    class="mt-3 rounded-2xl overflow-hidden bg-black/5 border aspect-[3/4] flex items-center justify-center">
                    <img
                      id="tryon-outfit-preview"
                      src=""
                      alt="Outfit Preview"
                      class="w-full h-full object-contain hidden" />
                    <div
                      id="tryon-outfit-placeholder"
                      class="text-xs text-muted p-3 text-center">
                      Preview akan muncul di sini
                    </div>
                  </div>
              ...
              <div class="mt-5 flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  class="flex-1 primary-bg text-white py-3 rounded-2xl font-medium hover:opacity-90 transition-opacity">
                ...
                <button
                  type="button"
                  onclick="resetTryOn()"
                  class="flex-1 border py-3 rounded-2xl font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  Reset
                </button>
              </div>
              ...
              <div class="mt-6">
                ...
                <div
                  class="rounded-2xl overflow-hidden border bg-black/5 aspect-[3/4] flex items-center justify-center">
                  <img
                    id="tryon-result"
                    src=""
                    alt="Try On Result"
                    class="w-full h-full object-contain hidden" />
                  <div
                    id="tryon-result-placeholder"
                    class="text-sm text-muted p-5 text-center">
            ...
            <div class="card-bg rounded-2xl p-5 shadow-md">
              ...
              <div class="mt-4 border-t pt-4">
                <button
                  type="button"
                  onclick="showToast('✅ Siap! Upload dulu 2 gambar ya.')"
                  class="w-full border rounded-2xl py-3 font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  Quick Guide
                </button>
              </div>
            </div>
        ...
        <!-- CHAT PAGE -->
        <div id="page-chat" class="page-content hidden h-full flex flex-col">
        ...
        <div
          id="page-settings"
          class="page-content hidden pb-20 lg:pb-0 fade-in">
  ...
  <script>
    ...
    function showPage(page) {
      ...
      // // Show/hide mobile nav bar and chat header
      // const navBar = document.getElementById("mobile-nav-bar");
      // const chatHeader = document.getElementById("chat-header-mobile");

      // if (page === "chat") {
      //   navBar.classList.add("hidden");

      //   // (Requirement #2) jangan tampilkan header chat mobile
      //   if (chatHeader) {
      ...
      // // Reset settings tabs when leaving settings
      // if (page !== "settings") {
      ...
      // // If open tryon, refresh wardrobe select
      // if (page === "tryon") {
      ...
      window.elementSdk.init({
        ...
        mapToCapabilities: (config) => ({
          recolorables: [{
            ...
            {
              get: () =>
                config.text_muted_color || defaultConfig.text_muted_color,
              set: (value) =>
                window.elementSdk.setConfig({
    ...
    function handleFeedback(button, type) {
      ...
      const likeBtn = button.parentElement.querySelector(
        "button:nth-child(1)",
      );
      const dislikeBtn = button.parentElement.querySelector(
        "button:nth-child(2)",
      );
    ...
    function handleTryOnUpload(event, type) {
      ...
      reader.onload = (e) => {
        ...
        if (type === "person") {
          ...
          document.getElementById("tryon-person-status").textContent =
            "Uploaded ✓";
        }
    ...
    function selectOutfitFromWardrobe() {
      ...
      if (!imageUrl) {
        showToast(
          "⚠️ Gambar wardrobe belum tersedia (upload ulang jika perlu).",
        );
        return;
      }
    ...
    (function() {
      function c() {
        ...
        if (b) {
          ...
          d.innerHTML =
            "window.__CF$cv$params={r:'9d6dbbc78798811b',t:'MTc3MjU5NDMwNS4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";
